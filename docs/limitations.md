...
These are the documented boundaries of the current version — the operating
envelope you can check your dataset against. They are **not** a commitment that
every item will be implemented: some are deliberate scope boundaries (handled by
downstream tools), others are possible future directions. The following are not
supported:

- **Read merging.** `PAIRED_END_SPLIT_PRIMERS` mode emits separate R1/R2 partial extractions; joined-count reconstruction requires bbmerge / vsearch / pear / flash and is not currently implemented.
